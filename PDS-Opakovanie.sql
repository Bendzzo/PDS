...
--*************************** CAS *******************************************************
-- 71. VypoËÌtajte vek kaûdej osoby na z·klade d·tumu narodenia.
...
-- VypÌöte vöetky osoby, ktorÈ sa narodili v nedeæu.
    select meno, priezvisko, ROD_CISLO,
...
-- VypÌöte poËet osÙb, ktorÈ sa narodili v priestupn˝ rok.
    select count(*) from p_osoba
    where to_char(to_date(substr(ROD_CISLO, 1, 2) || '-' || '02' || '-' || '28', 'RR-MM-DD') + 1, 'DD') = '29';

-- VypÌöte kvart·l (ötvrùrok), v ktorom sa narodilo najviac osÙb.
    select kvartal from (
...
-- VypÌöte vöetk˝ch poistencov, ktorÌ boli poistenÌ pred svojimi 18. narodeninami.
...
-- Zistite, v ktor˝ deÚ v t˝ûdni sa narodilo najviac osÙb.
    select den from(
...
-- VypÌöte osoby, ktorÈ dnes oslavuj˙ narodeniny.
...
-- Vytvorte pohæad, ktor˝ bude obsahovaù poistencov a poËet dnÌ, poËas ktor˝ch boli poistenÌ (dat_do - dat_od).
-- VypÌöte vöetky osoby, ktorÈ sa narodili v pracovn˝ deÚ (pondelok aû piatok).
-- Vytvorte pohæad, ktor˝ bude obsahovaù osoby a poËet dnÌ do ich najbliûöÌch narodenÌn.
